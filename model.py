...
class Siamese(nn.Module):
    def __init__(self):
        ...
        self.c1 = conv_block(3, 64, 3, 2, 1, True)
        ...
        self.fc = linear_block(32768, 1000, nn.ReLU)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.c2 = conv_block(64, 128, 3, 2, 1, True)
        self.c3 = conv_block(128, 256, 3, 2, 1, True)
        self.c4 = conv_block(256, 256, 3, 2, 1, True)
        # self.c5 = conv_block(256, 256, 3, 2, 1, True)
        self.t1 = uconv_block(256, 256, 3, 2, 1, 1)
        # self.tc1 = conv_block(128, 64, 3, 1, 1, True)
        self.t2 = uconv_block(512, 128, 3, 2, 1, 1)
        # self.tc2 = conv_block(64, 32, 3, 1, 1, True)
        self.t3 = uconv_block(256, 64, 3, 2, 1, 1)
        # self.tc3 = conv_block(32, 16, 3, 1, 1, True)
        self.t4 = uconv_block_final(128, 3, 3, 2, 1, 1)
    ...
    def forward_pass(self, X):
        ...
        X = torch.cat((self.t1(X_c4), X_c3), 1)
        # X = self.tc1(X)
        X = torch.cat((self.t2(X), X_c2), 1)
        # X = self.tc2(X)
        X = torch.cat((self.t3(X), X_c1), 1)
        # X = self.tc3(X)
        X = self.t4(X)
        # X = self.tc4(X)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.c1 = conv_block(3, 6, 3, 2, 1, False)
        self.c2 = conv_block(6, 12, 3, 2, 1, True)
        self.c3 = conv_block(12, 24, 3, 2, 1, True)
        self.c4 = conv_block(24, 24, 3, 2, 1, True)
        # self.c5 = conv_block(64, 128, 3, 2, 1, True)
        self.fc = linear_block(1536, 1, nn.Sigmoid)
...
def contrastive_loss(SX):
    ...
    loss = torch.clamp(10 - torch.norm(V, dim=0), min=0)
